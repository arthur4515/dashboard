...
export function AuthPage({ login, cadastro, avisar, erroConfig }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#F8F7FF] px-4 py-8 text-slate-800 dark:bg-[#0F0A1F] dark:text-slate-100">
      <div className="mx-auto flex min-h-[calc(100vh-64px)] max-w-6xl items-center">
        <div className="grid w-full gap-8 lg:grid-cols-[1fr_440px]">
          <section className="flex flex-col justify-center">
            <div className="brand-mark flex h-14 w-14 items-center justify-center rounded-lg">
              <WalletCards size={30} />
            </div>
            <h1 className="mt-6 max-w-2xl text-4xl font-black tracking-tight text-violet-950 dark:text-white sm:text-5xl">Dingaringa Finance</h1>
            <p className="mt-4 max-w-2xl text-lg text-slate-600 dark:text-slate-300">
              Um painel financeiro premium, roxo e inteligente para cuidar do dinheiro com leveza, sem perder a seriedade.
            </p>
          ...
          <section className="card p-6">
            <div className="mb-6 flex rounded-lg bg-violet-50 p-1 dark:bg-[#0F0A1F]">
              <button className={`flex-1 rounded-lg px-3 py-2 text-sm font-bold ${modo === 'login' ? 'bg-white text-violet-700 shadow-sm dark:bg-[#17102A]' : 'text-slate-500'}`} onClick={() => setModo('login')}>Entrar</button>
              <button className={`flex-1 rounded-lg px-3 py-2 text-sm font-bold ${modo === 'cadastro' ? 'bg-white text-violet-700 shadow-sm dark:bg-[#17102A]' : 'text-slate-500'}`} onClick={() => setModo('cadastro')}>Cadastrar</button>
            </div>
            ...
            <div className="mt-5 rounded-lg border border-violet-200 bg-violet-50 p-4 text-sm text-violet-900 dark:border-violet-900 dark:bg-violet-950 dark:text-violet-100">
