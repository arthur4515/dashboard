...
export function Relatorios({ estado, avisar }: { estado: AppState; avisar: (msg: string, tipo?: 'sucesso' | 'erro' | 'info') => void }) {
  const evolucao = evolucaoComAtual(estado, mesAtualISO()).filter((item) => item && typeof item.receita === 'number' && typeof item.despesa === 'number');
  const dadosMensais = evolucao.map((item) => ({
    ...item,
    economia: item.receita - item.despesa,
  }));
  ...
  if (estado.transacoes.length === 0 || dadosMensais.length === 0) {
    return (
    ...
    <div className="space-y-6">
      <Section titulo="Relatorios" acao={<div className="flex gap-2"><button className="btn-secondary" onClick={exportarCSV}><Download size={18} />CSV</button><button className="btn-secondary" onClick={() => avisar('PDF em desenvolvimento.', 'info')}><FileText size={18} />PDF</button></div>}>
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card titulo="Relatorio anual" valor={formatarMoeda(dadosMensais.reduce((t, i) => t + i.economia, 0))} texto="Saldo acumulado" />
        ...
        <Section titulo="Receitas, despesas e economia por mes">
          <ChartFrame className="h-[340px]">
            <BarChart data={dadosMensais}>
              ...
              <Bar dataKey="economia" name="economia" fill="#7C3AED" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ChartFrame>
        </Section>
            ...
            <tbody>
              {dadosMensais.map((item) => (
                <tr key={item.mes} className="border-t border-slate-100 dark:border-slate-800">
                  <td className="p-3 font-semibold">{item.mes}</td><td>{formatarMoeda(item.receita)}</td><td>{formatarMoeda(item.despesa)}</td><td>{formatarMoeda(item.economia)}</td><td>{formatarMoeda(item.patrimonio)}</td>
                </tr>
              ))}
            </tbody>
          </table>
